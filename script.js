...
let fetchProductData = async (checked = []) => {
  ...
  try {
    ...
    totalResultsElement.textContent = `${productListData.length} Results`;
    setupFilters();
    applyFiltersAndSort();
    setupSort();
  } catch (error) {
...
function applyFiltersAndSort() {
  ...
  filteredProductListData = productListData.filter((item) =>
      ...
      (element) =>
        `<div class="single-product">
               <div><img class="image-product" src=${
                ...
                <a class="truncate" href="/productDetail.html" onclick='setProductToLocalStorage("${
                  element.id
                }")'>${element.title}</a>
                <p>$ ${element.price}</p>
                </div>
